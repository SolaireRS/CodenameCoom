...
public class OsrsSpriteLoader {
	...
	public static void loadSprites() {
		try {
			Buffer index = new Buffer(FileOperations.readFile(Signlink.getCacheDirectory() + "233Osrssprites.idx"));
			Buffer data = new Buffer(FileOperations.readFile(Signlink.getCacheDirectory() + "233Osrssprites.dat"));
			...
			if (cache == null) {
				cache = new OsrsSpriteLoader[totalSprites];
				sprites = new Sprite[totalSprites];
			}
			for (int i = 0; i < totalSprites; i++) {
				int id = indexFile.readInt();
				if (cache[id] == null) {
					cache[id] = new OsrsSpriteLoader();
				}
	 ...
	 * @param sprite
	 */
	public static void createSprite(OsrsSpriteLoader sprite) {
		if (Configuration.DUMP_SPRITES) {
			File directory = new File(Signlink.getCacheDirectory() + "Sprites/OSrsdump/");
			if (!directory.exists()) {
				directory.mkdir();
			}
			DataUtils.writeFile(
	...
	public static OsrsSpriteLoader[] cache;
