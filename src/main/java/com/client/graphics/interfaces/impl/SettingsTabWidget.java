...
public class SettingsTabWidget extends RSInterface {
	...
	public static void audioSettings(RSInterface audio, TextDrawingArea[] tda) {
	    Interfaces.addSprite3(42531, 56);
	    musicVolumeSlider = slider(MUSIC_SLIDER, 0, 10, 37, 54, 3); // Back to 0, 10
	    Interfaces.addSprite3(42533, 52);
	    soundVolumeSlider = slider(SOUND_SLIDER, 0, 10, 37, 54, 4); // Back to 0, 10
	    Interfaces.addSprite3(42535, 53);
	    areaSoundVolumeSlider = slider(AREA_SOUND_SLIDER, 0, 10, 37, 54, 5); // Back to 0, 10
	...
	public static void updateSettings() {
		...
		// Set slider positions from saved preferences
		RSInterface.interfaceCache[ZOOM_SLIDER].slider.setValue(Client.cameraZoom);
		RSInterface.interfaceCache[BRIGHTNESS_SLIDER].slider.setValue(Preferences.getPreferences().brightness);
		...
		RSInterface.interfaceCache[AREA_SOUND_SLIDER].slider.setValue(Preferences.getPreferences().areaSoundVolume);
		
		// Apply the volumes immediately
		Client.instance.setMusicVolume((int) Preferences.getPreferences().musicVolume);
		Client.instance.setSoundVolume((int) Preferences.getPreferences().soundVolume);
		Client.instance.setAreaSoundVolume((int) Preferences.getPreferences().areaSoundVolume);
	}
