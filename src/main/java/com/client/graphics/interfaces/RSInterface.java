...
public class RSInterface {
	...
	public Model get_if_model(AnimationDefinition seq, int k, boolean active) {
		Model model;
		if (active)
		...
		if (model == null)
			return null;
		if (k == -1 && seq == null && model.colors == null)
			return model;
		if (k != -1 || seq != null)
			model.apply_label_groups();
		if (seq != null)
			seq.animate_iftype_model(model, k);
	...
	public static void addClickableTextmusic(int id, String text, String tooltip, TextDrawingArea tda[], int idx, int color,
										boolean center, boolean shadow, int width) {
		RSInterface tab = addTabInterface(id);
		tab.parentID = id;
		tab.id = id;
