...
public class DrawingArea extends NodeSub {
	// GPU Integration Fields
	private static GPUPlugin gpuPlugin;
	private static boolean gpuIntegrationEnabled = false;
	...
	public static void initDrawingArea(int i, int j, int[] ai, float[] depth) {
		...
		GPUDrawingAreaIntegration.initializeGPUContext(j, i);

	}
	public static void renderFrame() {
	    if (gpuIntegrationEnabled) {
	        GPUDrawingAreaIntegration.renderFrame();
	    }
	    
	    // Add this line to apply GPU effects at the end of each frame
	    Rasterizer.finalizeFrame();
	}
	public static boolean isGPUEnabled() {
		return gpuIntegrationEnabled && GPUDrawingAreaIntegration.isGPUEnabled();
	}
	...
	public static void fillRectangle(int x, int y, int width, int height, int colour) {
		...
		public static void setGPUPlugin(GPUPlugin plugin) {
			gpuPlugin = plugin;
			GPUDrawingAreaIntegration.setGPUPlugin(plugin);
			gpuIntegrationEnabled = true;
			System.out.println("GPU Plugin integrated with DrawingArea");
		}
