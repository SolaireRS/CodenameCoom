...
public final class OnDemandFetcher extends OnDemandFetcherParent implements Runnable {
	// In OnDemandFetcher.java, add these public methods:
	public static int[] getMapIndices1() {
	    return mapIndices1;
	}

	public static int[] getMapIndices2() {
	    return mapIndices2;
	}

	public static int[] getMapIndices3() {
	    return mapIndices3;
	}
	private void readData() {
		try {
			int j = inputStream.available();
			if (expectedSize == 0 && j >= 7) {
				waiting = true;
...
//					throw new RuntimeException("buffer overflow!");
	...
	public int getMapFiles(int mapType, int y, int x) {
		...
		for (int j1 = 0; j1 < mapIndices1.length; j1++) {
			if (mapIndices1[j1] == mapFileHash) {
				if (mapType == 0) {
					return mapIndices2[j1] > 17000 ? -1 : mapIndices2[j1];
				} else {
					return mapIndices3[j1] > 17000 ? -1 : mapIndices3[j1];
				}
			}
		}
		return -1;
	...
	public OnDemandFetcher() {
		...
		gzipInputBuffer = new byte[999999999];
